...
from consolemenu.items import FunctionItem, SubmenuItem, CommandItem, ExternalItem
            ...
            for jobnumber in grouped_month.keys():
                for activitynumber in grouped_month[jobnumber].keys():
                    for taskname in grouped_month[jobnumber][activitynumber].keys():
                        ...
                        if hours == 0:
                            continue
                        print(f"    job={jobnumber}, activity={activitynumber}, task={taskname}, {hours=:.1f}, type={bonus_type}", file=output_file)
    ...
    for year in records.keys():
        ...
        for month in records[year].keys():
            if month not in report['years'][year]['months']:
                report['years'][year]['months'][month] = {
                    'records':[]
                }
            rlon_billable = 0
            ...
            if monthly_bonus_hours >= rtotal_required_hours:
                ...
                if monthly_bonus_hours + rtot_bank >= rtotal_required_hours:
                    rtot_count += 1
                    report['years'][year]['months'][month]['Rtotal'] = 'True, using %.1fh from hour bank' % (rtotal_required_hours - monthly_bonus_hours)
        ...
        if year_bonus_hours >= yearly_hours - 40:
            if year_bonus_hours >= yearly_hours:
                ...
                if rtot_count < 11:
                    report['years'][year]['info']['Rtotal'] = "Within 40 hours of all hours in year, meaning you got %d retroactively" % (11 - rtot_count)
                else:
                    report['years'][year]['info']['Rtotal'] = "Within 40 hours of all hours in year, but already received 11 Rtotal bonuses, so no adjustment made"
                rtot_count = 11
        else:
            report['years'][year]['info']['Rtotal'] = "You have %d bonus hours, you need %d (yearly hours - 40) to qualify for retroactive RTotal" % (year_bonus_hours, (yearly_hours - 40))
...
def edit_timesheet_line_single_menu(record):
    ...
    for bonus_type in bonus_types:
        fn_item = FunctionItem(f"Update classification to {bonus_type}", update_record_bonus_type, args=[line_menu, record, bonus_type])
...
def edit_timesheet_lines_menu():
    ...
    for jobnumber in state['unknown']:
        for activitynumber in state['unknown'][jobnumber]['activities']:
            ...
            record = create_record(jobnumber, activitynumber, None, state['unknown'][jobnumber]["activities"][activitynumber]['hours'], 'unknown')
...
def login(menu):
    ...
    timesheet_lines_classification_menu = ClearingSubmenuItem("Inspect timesheet lines classifications", timesheet_lines_menu, main_menu)
